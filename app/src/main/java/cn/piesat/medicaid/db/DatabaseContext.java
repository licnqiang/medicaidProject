 ...
 * author: lq
 * created on: 2018/7/3.
 * description:用于支持对存储在SD卡上的数据库的访问
 **/
public class DatabaseContext extends ContextWrapper {
     ...
     */
    public DatabaseContext(Context base, File file, boolean isUseDefaultDB) {
        super(base);
        this.file = file;
        this.context = base;
    ...
    public boolean createInitDatabase(File toFile, String name, Context context) {
        ...
        try {
            is = context.getResources().getAssets().open("AppDatabase.db");
            // 创建输出流
